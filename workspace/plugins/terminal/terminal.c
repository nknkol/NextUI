...
typedef struct TerminalSession {
    int id;
    char name[64];
    bool active;      // 标记会话是否在列表中激活
    bool initialized; // 标记是否已初始化为真实终端
    pid_t pid;
    int ptm_fd;
...
static bool initialize_real_terminal(TerminalSession* session) {
    if (session->initialized) return true;

    int ptm_fd = -1, pts_fd = -1;
    pid_t pid;

    if (openpty(&ptm_fd, &pts_fd, NULL, NULL, NULL) < 0) return false;

    pid = fork();
    if (pid < 0) { close(ptm_fd); close(pts_fd); return false; }
    ...
    if (tsm_screen_new(&session->screen, NULL, NULL) != 0) { close(ptm_fd); kill(pid, SIGKILL); return false; }
    if (tsm_vte_new(&session->vte, session->screen, vte_write_cb, session, NULL, NULL) != 0) { tsm_screen_unref(session->screen); close(ptm_fd); kill(pid, SIGKILL); return false; }
...
static void handle_list_input(void) {
    ...
    else if (PAD_justRepeated(BTN_UP) && session_count > 0) { 
        int prev = selected_index;
        do { selected_index = (selected_index - 1 + MAX_SESSIONS) % MAX_SESSIONS; } 
        while (!sessions[selected_index].active && selected_index != prev);
    }
    else if (PAD_justRepeated(BTN_DOWN) && session_count > 0) {
        int prev = selected_index;
        do { selected_index = (selected_index + 1) % MAX_SESSIONS; }
        while (!sessions[selected_index].active && selected_index != prev);
    }
...
static int plugin_init(void* main_screen) {
    ...
    if (!exists(mono_font_path)) { snprintf(mono_font_path, sizeof(mono_font_path), "%s/font.ttf", RES_PATH); }
    mono_font = TTF_OpenFont(mono_font_path, SCALE1(TERMINAL_FONT_HEIGHT - 2));
    if (!mono_font) return -1;
