...
# all: setup $(PLATFORMS) done #special #package done
...
setup: name
	...
	echo $(BUILD_HASH) > ./workspace/other/readmes/hash.txt
	
	# copy readmes to workspace so we can use Linux fmt instead of host's
# 	mkdir -p ./workspace/readmes
	cp ./skeleton/BASE/README.txt ./workspace/other/readmes/BASE-in.txt
	cp ./skeleton/EXTRAS/README.txt ./workspace/other/readmes/EXTRAS-in.txt
...
package: tidy
	...
	cp ./workspace/other/readmes/BASE-out.txt ./build/BASE/README.txt
	cp ./workspace/other/readmes/EXTRAS-out.txt ./build/EXTRAS/README.txt
	rm -rf ./workspace/other/BASE-out.txt
	rm -rf ./workspace/other/EXTRAS-out.txt
